...
- `beaconDetected`, callback `() => void`: This event is triggered when a beacon sets in the SDK is detected. It is triggered exclusively if the trip recording has not been started yet (i.e. during the `INACTIVE` state).
- `beaconConfirmed`, callback `(beacon: BeaconData) => void`: Called when a beacon set in the SDK is detected and confirmed during a trip. It is triggered exclusively if the trip recording has started (i.e. during the `RUNNING` state).
